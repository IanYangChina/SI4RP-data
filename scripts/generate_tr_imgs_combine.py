...
#                     # plt.tight_layout()
#                     plt.savefig(os.path.join(data_path, f'img_combine.pdf'), bbox_inches='tight', pad_inches=0, dpi=500)
#                     plt.close(fig)

for agent in ['rectangle', 'round', 'cyl']:
    for data_ind in [0, 1]:
        pic_path = os.path.join(script_path, '..', 'real-robot-pics', f'{agent}-{data_ind}')
        fig, axes = plt.subplots(1, 11, figsize=(11 * 2, 2))
        plt.subplots_adjust(wspace=0.01)
        for i in range(11):
            img = plt.imread(os.path.join(pic_path, f'{i+1:02d}.png'))
            axes[i].imshow(img)
            # axes[i].set_xlabel(f't{img_id}'.translate(SUB))
            axes[i].set_xticks([])
            axes[i].get_yaxis().set_visible(False)
            axes[i].set_frame_on(False)
        # plt.tight_layout()
        plt.savefig(os.path.join(pic_path, f'tr_img_combine-{data_ind}.pdf'), bbox_inches='tight', pad_inches=0, dpi=500)
        plt.close(fig)