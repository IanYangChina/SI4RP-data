...
e = np.array([1], dtype=np.float32)  # Young's modulus
nu = np.array([0.2], dtype=np.float32)  # Poisson's ratio
yield_stress = np.array([2.5], dtype=np.float32)
...
print(f'Chamfer loss: {mpm_env.loss.total_loss[None]}')
t2 = time()
...
real_pcd = o3d.io.read_point_cloud(os.path.join(data_path, 'pcd_' + data_ind+str(1) + '.ply'))
...
o3d.visualization.draw_geometries([sim_pcd, real_pcd, frame])
